...
public_url() { local internal="$1"; local scheme="${2:-http}"; local host="${PUBLIC_IPADDR:-localhost}"; local port="$(ext_port "$internal")"; echo "${scheme}://${host}:${port}"; }
...
qlog "ENV SUMMARY: DISPLAY=${DISPLAY:-:1} VNC_PORT=${VNC_PORT:-6901} HEALTH_PORT=${HEALTH_PORT:-8686} RVC_PORT=${RVC_PORT:-7865}"
}
